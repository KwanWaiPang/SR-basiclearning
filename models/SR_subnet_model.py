...
class SRModel(BaseModel):
    ...
    def __define_grad_params(self, finetune_type=None):##############define which Parameters to be optimized
    ...
    def optimize_parameters(self, step):
        ...
        #l_pix_noise = self.l_pix_noise_w * self.cri_pix_noise(self.fake_noise, self.real_noise)
        ...
        self.fake_noise = self.subnet(self.var_L)
        self.fake_H = self.netG(self.fake_noise)
        #self.fake_H = self.netG(torch.cat((self.var_L, self.fake_noise), 1))
        #self.fake_H = self.netG((self.var_L, self.fake_noise))
    ...
    def get_current_visuals(self, need_HR=True):
        ...
        ###############
        #out_dict['ns'] = self.fake_noise.detach()[0].float().cpu()
        if need_HR:
